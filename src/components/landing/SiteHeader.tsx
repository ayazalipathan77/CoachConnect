...
export async function SiteHeader() {
  ...
  const dashHref = user?.role === 'coach' ? '/dashboard/coach' : user?.role === 'admin' ? '/admin' : '/dashboard';
      ...
      <div className="w-full flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-4">
          {user ? (
            ...
            <>
              <Link href="/login" className="text-sm font-bold text-white hover:text-brand transition-colors">
                Log In
              </Link>
        ...
        {/* Mobile hamburger — renders drawer client-side */}
        <MobileNav user={mobileUser} dashHref={dashHref} />
      </div>
    </header>
  );
}
