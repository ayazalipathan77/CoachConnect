...
import { updateClientAvatar, type ClientProfileState } from '@/server/client/actions';

export function ClientAvatarUpload({ currentImage }: { currentImage: string | null }) {
  const formRef = useRef<HTMLFormElement>(null);
  const inputRef = useRef<HTMLInputElement>(null);
  const hiddenRef = useRef<HTMLInputElement>(null);

  const [state, action, pending] = useActionState<ClientProfileState, FormData>(updateClientAvatar, undefined);
  ...
  function handleFileChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    reader.onload = () => {
      if (hiddenRef.current) hiddenRef.current.value = reader.result as string;
